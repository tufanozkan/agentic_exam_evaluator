...
function QuestionResultCard({ result }: { result: GradingResultPayload }) {
  ...
  {isLoading && <p className="text-sm text-gray-500 mt-2 flex items-center"><Clock size={14} className="animate-spin mr-1"/> Yapay zeka düşünüyor...</p>}

        <form onSubmit={handleFollowUp} className="flex items-center space-x-2 pt-2">
          <input type="text" value={currentQuery} onChange={(e) => setCurrentQuery(e.target.value)} placeholder="Yapay Zekaya soru sor..." className="flex-grow p-2 border border-gray-300 rounded-md text-sm bg-black text-white" disabled={isLoading} />
      ...
      <details className="mt-3">
        <summary className="cursor-pointer text-xs text-yellow-700">Teknik Detayları Göster</summary>
          ...
          <div key={studentId} className="bg-white p-6 rounded-lg shadow">
            ...
            {studentSummary && (
               <div className="mt-6 bg-green-50 border-l-4 border-green-500 p-4 rounded">
                  <h3 className="font-bold mb-2 text-black">Öğrenci Özeti</h3>
